...
肩書はチームリーダーですが、配下にメンバーがいないため、  
実質的にはソロプレイヤーでシステム基盤全体の整備を行っています。  
期待されている役割としては、  

- 将来のサービス成長を見据えたシステム基盤の整備
...
●サービス基盤の整備  
会社のサービス全般に関わる、システム基盤の構成検討や構築、保守運用を担っています。  
新規サービスローンチや機能拡張の際は、マネージドサービスを積極的に用いるようにして、  
...
あとはアプリケーションを配備するだけでテスト可能となる仕組みを整備しました。  
...
TerraformやAnsibleを利用したインフラ構成のコード化を推進していますです。  
...
システム規模的には中規模程度でしたが、データセンターでのラッキング・ケーブリングなどの物理作業から、  
